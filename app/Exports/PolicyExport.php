    ...
    public function collection()
    {
       ...
       if($policies->count()){
        foreach ($policies as $key => $policy) {
            ...
            $data['Phone']=$policy->lead->phone ?? null;
            $data['Email']=$policy->lead->email?? null;
            $data['Insurance']=$policy->lead->insurances->name ?? null;
            $data['Product']=$policy->lead->products->name ?? null;
            $data['Sub Product']=$policy->lead->subProduct->name ?? null;
            ...
            $data['make']=$policy->make ?? null;
            $data['model']=$policy->model ?? null;
            ...
            $data['fuel_type']=$policy->fuel_type ?? null;
            $data['passenger_carrying_capacity']=$policy->passenger_carrying_capacity ?? null;
            $data['category']=$policy->category ?? null;
            $data['varriant']=$policy->varriant ?? null;
